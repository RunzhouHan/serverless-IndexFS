...
void MetaDBServiceClient::GetEntry(const KeyInfo_THRIFT& key, const StatInfo& info)
{
  send_GetEntry(key, info);
  recv_GetEntry();
}

void MetaDBServiceClient::send_GetEntry(const KeyInfo_THRIFT& key, const StatInfo& info)
{
  int32_t cseqid = 0;
...
void MetaDBServiceClient::recv_GetEntry()
{
...
void MetaDBServiceProcessor::process_GetEntry(int32_t seqid, ::apache::thrift::protocol::TProtocol* iprot, ::apache::thrift::protocol::TProtocol* oprot, void* callContext)
{
  ...
  try {
    iface_->GetEntry(args.key, args.info);
  } catch (UnrecognizedDirectoryError &unknown_dir) {
    result.unknown_dir = unknown_dir;
    result.__isset.unknown_dir = true;
  } catch (ServerRedirectionException &srv_redirect) {
...
void MetaDBServiceConcurrentClient::GetEntry(const KeyInfo_THRIFT& key, const StatInfo& info)
{
  int32_t seqid = send_GetEntry(key, info);
  recv_GetEntry(seqid);
}

int32_t MetaDBServiceConcurrentClient::send_GetEntry(const KeyInfo_THRIFT& key, const StatInfo& info)
{
  int32_t cseqid = this->sync_.generateSeqId();
  ::apache::thrift::async::TConcurrentSendSentry sentry(&this->sync_);
  oprot_->writeMessageBegin("GetEntry", ::apache::thrift::protocol::T_CALL, cseqid);
...
void MetaDBServiceConcurrentClient::recv_GetEntry(const int32_t seqid)
{
