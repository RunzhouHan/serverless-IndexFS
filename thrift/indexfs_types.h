...
class KeyInfo_THRIFT {
 public:
  ...
  KeyInfo_THRIFT(const KeyInfo_THRIFT&);
  KeyInfo_THRIFT& operator=(const KeyInfo_THRIFT&);
  KeyInfo_THRIFT() : parent_id_(0), partition_id_(0), file_name_() {
  }

  virtual ~KeyInfo_THRIFT() throw();
  int64_t parent_id_;
  int16_t partition_id_;
  std::string file_name_;

  void __set_parent_id_(const int64_t val);

  void __set_partition_id_(const int16_t val);

  void __set_file_name_(const std::string& val);

  bool operator == (const KeyInfo_THRIFT & rhs) const
  {
    if (!(parent_id_ == rhs.parent_id_))
      return false;
    if (!(partition_id_ == rhs.partition_id_))
      return false;
  ...
  bool operator < (const KeyInfo_THRIFT & ) const;
...
inline std::ostream& operator<<(std::ostream& out, const KeyInfo_THRIFT& obj)
{
  obj.printTo(out);
  return out;
}
