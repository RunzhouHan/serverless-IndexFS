...
# endif
 ...
 public:
  ...
  newTest(int my_rank, int comm_sz)
  ...
  virtual void Run() {
	...
	if (d >= num_dirs_)
        ...
        try {
	  //MakeDirectory(IO_, listener_, i%2);
	  CreateFile(IO_, listener_, d, i);
          GetAttr(IO_, listener_, d, i);
          //IO->MakeDirectory(d, FLAGS_prefix);
	  //IO->NewFile(d, i, FLAGS_prefix);
	  //IO->GetAttr(d, i, FLAGS_prefix);
#         ifndef NDEBUG
...
IOTask* IOTaskFactory::GetnewTestTask(int my_rank, int comm_sz) {
  return new newTest(my_rank, comm_sz);
}
